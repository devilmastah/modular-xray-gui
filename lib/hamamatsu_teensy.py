#!/usr/bin/env python3
"""
HamamatsuTeensy hardware library.
...
class HamamatsuTeensy:
  ...
  STRUCT_STATE = struct.Struct("<BIIBIIII")   # extended (26 B): + dma_ok_count, dma_timeout_count
  STRUCT_STATE_LEGACY = struct.Struct("<BIIBII")  # backup firmware (18 B): no dma_ok/dma_timeout
  ...
  # Full resolution: 2400x2400, 12-bit packed (8.64 MB; requires 16MB PSRAM on Teensy)
  FRAME_WIDTH = 2400
  FRAME_HEIGHT = 2400
  ...
  def get_state(self):
    ...
    if len(dat) == self.STRUCT_STATE.size:
      u = self.STRUCT_STATE.unpack(dat)
      return {'state': u[0], 'row': u[1], 'col': u[2], 'stop_reason': u[3], 'dma_wait': u[4], 'overhead_max_us': u[5], 'dma_ok': u[6], 'dma_timeout': u[7]}
    if len(dat) == self.STRUCT_STATE_LEGACY.size:
      u = self.STRUCT_STATE_LEGACY.unpack(dat)
      return {'state': u[0], 'row': u[1], 'col': u[2], 'stop_reason': u[3], 'dma_wait': u[4], 'overhead_max_us': u[5], 'dma_ok': 0, 'dma_timeout': 0}
    raise ValueError(f"Response does not match expected struct size: {len(dat)} (expected {self.STRUCT_STATE.size} or {self.STRUCT_STATE_LEGACY.size})")
