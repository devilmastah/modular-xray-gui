...
void usb_custom_reset_bulk_state(void) {
  bulk_data = NULL;
  bulk_len_remaining = 0;
}
...
void usb_custom_init_bulk_transfer(uint8_t *buffer, uint32_t len, uint8_t transfer_id) {
  ...
  // Flush only the chunk we're about to send (caller already flushed full buffer after capture;
  // flushing 8MB here would block 1–2s and cause host control read to timeout)
  arm_dcache_flush_delete(bulk_data, transfer_len);
